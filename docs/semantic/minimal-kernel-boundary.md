# APEX Minimal-Kernel Boundary

## Status and scope

This document is a **boundary extraction**, not a declaration that a canonical
kernel has been selected. It compresses the first-order contradictions among the
...
The minimal boundary is the meet of compatible, shared constraints—not the
union of every rule asserted by every artifact:

\[
K_{min} = \bigwedge_i Compatible(C_i)
\]

A rule excluded from \(K_{min}\) is not necessarily wrong. It becomes a policy
...
Transformation  --preserves> Request + K_min constraints
```
...
The following constraints form the proposed shared meet, \(K_{min}\). They are
...
| `K-12` | Conservative transformations preserve `Request` and all applicable `K_min` constraints. |
...
A degree of freedom is eliminable from the kernel when changing it does not
change the shared primitive meanings or any `K_min` verdict.
...
A \sim_{K_{min}} B
\iff
\forall x,\ Verdict_{K_{min}}(A,x)=Verdict_{K_{min}}(B,x)
\]

The quotient by \(\sim_{K_{min}}\) removes presentation and orchestration
...
operations and therefore are not kernel axioms.
...
3. **Test necessity.** Remove the rule temporarily. If all `K_min` verdicts and
...
- Two different header layouts can receive identical `K_min` verdict vectors.
...
- No APEX verdict is inferred from MCMC tests, and no MCMC result is inferred
  from APEX doctrine.
- A new artifact cannot claim kernel authority merely by restating the shared
...
The minimal kernel is therefore not one of the current APEX documents. It is the
shared typed boundary described above. The next valid operation is **type
unification design**: define a machine-readable schema for the primitives,
verdict algebra, authority qualifications, policy fibers, and release profiles,
then mechanically test each current artifact against that schema.
